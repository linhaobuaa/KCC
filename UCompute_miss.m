function util = UCompute_miss(index,U,w,C,n,r,K,sumKi,Pvector,M)
%==========================================================================
% FUNCTION: util = UCompute_miss(index,U,w,C,n,r,K,sumKi,Pvector,M)
% DESCRIPTION: This function performs utility calculating for consensus
%              clustering on dataset with missing values
%
...
%==========================================================================
    ...
    Pci = zeros(K,r); % initialize cluster distribution for each basic partition
    numi = sum(M); % calculate the number of non-missing members in each basic partition
    for i=1:r
        Pci(:,i) = gClusterDistribution2(index(M(:,i)),K,numi(i));
    end
    ratioi = numi/n; % calculate the ratio of non-missing members in each basic partition
    
    wei = w.*ratioi'; % calculate adjusted weight
    ...
    switch lower(U{1,1}) % U{1,1} denotes the type of the utility function
        case 'u_c'
            ...
            if strcmp(U{1,2},'std')==1 % U{1,2} indicates whether to use the normalized utility function
                ...
                util(1,1) = (sum(Pci.*Cmatrix)-Pvector)*(wei./Pvector');
                util(2,1) = (sum(Pci.*Cmatrix)-Pvector)*(wei./sqrt(Pvector'))/sqrt(sum(Pc.^2));
            end
        case 'u_h'
            for i=1:r
                tmp = C(:,(sumKi(i)+1):sumKi(i+1));
             ...
             if strcmp(U{1,2},'std')==1
                 ...
                 util(1,1) = (sum(Pci.*Cmatrix)+Pvector)*(wei./Pvector'); 
                 util(2,1) = (sum(Pci.*Cmatrix)+Pvector)*(wei./sqrt(Pvector'))/sqrt(-sum(Pc.*log2(Pc)));
             end            
        case 'u_cos'
            for i=1:r
                tmp = C(:,(sumKi(i)+1):sumKi(i+1));
            ...
            if strcmp(U{1,2},'std')==1
                ...
                util(1,1) = (sum(Pci.*Cmatrix)-Pvector)*(wei./Pvector');
                util(2,1) = (sum(Pci.*Cmatrix)-Pvector)*(wei./sqrt(Pvector'))/sqrt(sqrt(sum(Pc.^2)));
            end            
        case 'u_lp'
            p = U{1,3};
            for i=1:r
            ...
            if strcmp(U{1,2},'std')==1
                ...
                util(1,1) = (sum(Pci.*Cmatrix)-Pvector)*(wei./Pvector');
                util(2,1) = (sum(Pci.*Cmatrix)-Pvector)*(wei./sqrt(Pvector'))/sqrt(sum(Pc.^p)^(1/p));
            end            
        otherwise
            error('UCompute_miss:UnknowUtilityFunction',...
